...
agent_mc1=computer_MC(8,8)
agent_mc2=computer_MC(8,8)
...
def main():
    ...
    if proc==0:
        ...
        if turn==0:
            ...
            cx,cy=agent_mc1.action(color[turn],100,board)
            ishi_utsu(int(cx),int(cy),color[turn])
            space-=1
            proc=3
        else: 
            #cx,cy=agent_nn.select_action(board)#computer_2(color[turn],200)
            cx,cy=agent_mc2.action(color[turn],100,board)
            ishi_utsu(int(cx),int(cy),color[turn])
            space-=1
            proc=3
    elif proc==3:
...
root.after(100,main)
root.mainloop()
agent_mc1.save_wins(agent_mc1.win_probs_thistime)
agent_mc2.save_wins(agent_mc2.win_probs_thistime)